                    ...
                    <div class="row align-items-center justify-content-between g-5">
                        ...
                        <div class="col-12 col-md-6" >
                            <form action="{{ route('items.index') }}" method="GET">
                                <div class="row g-2 align-items-end custom-margin-mobile">
                                    <!-- Input Pencarian -->
                                    ...
                                    <div class="col-12 col-md-auto d-flex flex-wrap gap-2">
                                        <button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#advanced-filter" aria-expanded="false" aria-controls="advanced-filter">
                                ...
                                <!-- Advanced Filter -->
                                <div class="collapse mt-3" id="advanced-filter">
                                <div class="row g-3">
                                    <div class="col-md-4">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="number" name="min_price" id="min_price" class="form-control" value="{{ old('min_price', request('min_price', $min_price ?? '')) }}" min="0" step="1000" placeholder="Min Harga">
                                    </div>

                                    <div class="col-md-4">
                                        <label for="max_price" class="form-label">Harga Maksimum (Rp)</label>
                                        <input type="number" name="max_price" id="max_price" class="form-control" value="{{ old('max_price', request('max_price', $max_price ?? '')) }}" min="0" step="1000" placeholder="Max Harga">
                                    </div>
                            ...
                            <table class="table align-items-center mb-0 table-striped">
                                <thead class="table-dark">
                                    <tr class="text-center">
                                        ...
                                        <th class="text-uppercase text-xxs font-weight-bolder opacity-7" colspan="2">Harga</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="ps-4 text-center" style="width: 100px">
                                        ...
                                        <td class="text-end" style="width:20px">
                                            <p class="text-xs font-weight-bold mb-0">
                                                {{ number_format($item['branchPrice'] ?? '-', 0, ',', '.') }}
                                            </p>
                                        </td>
                                        <td class="text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ $item['availableToSell'] ?? '-' }}</p>
                                        </td>
                                        <td class="align-middle text-center"><a href="{{ route('items.detail', ['id' => $item['id']]) }}" class="btn btn-warning btn-sm">Detail</a></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                            ...
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-center mb-0">
                                    ...
                                    <li class="page-item {{ $pagination->current_page == 1 ? 'disabled' : '' }}">
                                        <a class="page-link" href="{{ $pagination->prev_page_url }}&q={{ request('q') }}&stok_ada={{ request()->has('stok_ada') ? request('stok_ada') : '' }}&id={{ request('id') }}&min_price={{ request('min_price') }}&max_price={{ request('max_price') }}" aria-label="Previous">
                                    ...
                                    {{-- First Page Link --}}
                                        @if($start > 1)
                                        <li class="page-item">
                                            <a class="page-link" href="{{ route('items.index', ['page' => 1, 'q' => request('q'), 'stok_ada' => request()->has('stok_ada') ? request('stok_ada') : '', 'id' => request('id'), 'min_price' => request('min_price'), 'max_price' => request('max_price')]) }}">1</a>
                                        </li>
                                            @if($start > 2)
                                    ...
                                    @for ($i = $start; $i <= $end; $i++)
                                        <li class="page-item {{ $pagination->current_page == $i ? 'active' : '' }}">
                                            <a class="page-link" href="{{ route('items.index', ['page' => $i, 'q' => request('q'), 'stok_ada' => request()->has('stok_ada') ? request('stok_ada') : '', 'id' => request('id'), 'min_price' => request('min_price'), 'max_price' => request('max_price')]) }}">{{ $i }}</a>
                                        </li>
                                    @endfor
                                    ...
                                    @if($end < $pagination->last_page)
                                        ...
                                        @endif
                                        <li class="page-item">
                                            <a class="page-link" href="{{ route('items.index', ['page' => $pagination->last_page, 'q' => request('q'), 'stok_ada' => request()->has('stok_ada') ? request('stok_ada') : '', 'id' => request('id'), 'min_price' => request('min_price'), 'max_price' => request('max_price')]) }}">
                                    ...
                                    <li class="page-item {{ $pagination->current_page == $pagination->last_page ? 'disabled' : '' }}">
                                        <a class="page-link" href="{{ $pagination->next_page_url }}&q={{ request('q') }}&stok_ada={{ request()->has('stok_ada') ? request('stok_ada') : '' }}&id={{ request('id') }}&min_price={{ request('min_price') }}&max_price={{ request('max_price') }}" aria-label="Next">
